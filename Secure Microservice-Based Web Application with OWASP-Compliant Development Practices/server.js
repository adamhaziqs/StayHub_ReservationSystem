...
try {
  ...
  console.error('❌ CRITICAL ERROR: Failed to create secure storage directories. Check folder permissions:', err.message);
  process.exit(1);
}
...
function getRandomFileName(originalName) {
  ...
  // Use random bytes for unique, unpredictable filenames (compatible with all Node.js versions)
...
app.get('/uploads/:fileName', ensureAuthenticated, (req, res) => {
  try {
    ...
    // Check file is in upload directory to prevent path traversal
